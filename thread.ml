...
let open_connection sockaddr =
  let sock = Lwt_unix.socket (Unix.domain_of_sockaddr sockaddr) Lwt_unix.SOCK_STREAM 0 in
    ...
    >>= fun () -> Lwt_unix.set_close_on_exec sock;
...
let output_binary_int oc n =
  ...
  >>= fun () -> output_char oc (Char.chr ((n lsr 16) land 255))
  >>= fun () -> output_char oc (Char.chr ((n lsr 8) land 255))
  >>= fun () -> output_char oc (Char.chr (n land 255))
